...
This repository inherits the broader cross-project rules in `AI_PROJECT_CANON.md`. This file defines the repo-specific implementation of those rules.

## Stack

- Use Tauri for the desktop shell.
- Use React + TypeScript + Vite for the frontend.
- Use Tailwind CSS and shadcn/ui-style primitives for the UI.
- Use Playwright for frontend validation and native smoke validation where relevant.

## Project defaults

- Keep native logic in small Rust modules.
- Keep UI logic in testable TypeScript where possible.
- Reuse and extend existing abstractions safely before introducing new ones.
- Use current official docs for Tauri, frontend libraries, and platform-sensitive work.

## Mandatory workflow

- Follow TDD for behavior changes.
- Validate frontend work with Playwright.
- Validate native logic with Rust tests.
- Validate startup-critical flows with smoke or platform tests where supported.
- Do not claim success before the relevant validation paths pass.
